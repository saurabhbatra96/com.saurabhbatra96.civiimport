 ...
 */
function _civiimport_civix_civicrm_xmlMenu(&$files) {
  foreach (_civiimport_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 * @return array(string)
 */
function _civiimport_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_civiimport_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
 ...
 */
function _civiimport_civix_civicrm_managed(&$entities) {
  $mgdFiles = _civiimport_civix_find_files(__DIR__, '*.mgd.php');
  foreach ($mgdFiles as $file) {
    $es = include $file;
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'com.saurabhbatra96.civiimport';
      }
      $entities[] = $e;
    }
  }
 ...
 */
function _civiimport_civix_civicrm_caseTypes(&$caseTypes) {
  if (!is_dir(__DIR__ . '/xml/case')) {
    return;
  }

  foreach (_civiimport_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
    ...
    $caseTypes[$name] = array(
      'module' => 'com.saurabhbatra96.civiimport',
      'name' => $name,
      'file' => $file,
    );
  }
 ...
 */
function _civiimport_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _civiimport_civix_glob(__DIR__ . '/ang/*.ang.php');
  foreach ($files as $file) {
    $name = preg_replace(':\.ang\.php$:', '', basename($file));
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'com.saurabhbatra96.civiimport';
    }
 ...
 */
function _civiimport_civix_glob($pattern) {
  $result = glob($pattern);
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civiimport_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
 ...
 */
function _civiimport_civix_civicrm_alterSettingsFolders(&$metaDataFolders = NULL) {
  static $configured = FALSE;
  if ($configured) {
    return;
  }
